...
export default function BlogPostLayout({ post, relatedPosts, schemas }) {
  ...
  const challengeSection = post.sections.find((section) =>
    section.title.toLowerCase().includes("challenge")
  );
      ...
      <article className="flex min-h-screen flex-col items-center justify-start overflow-x-clip px-4 py-28 md:px-8 md:py-40 xl:px-10">
        ...
        {post.calculatorCta && (
          <WebsitePriceCalculator
            title="Estimate your website cost"
            description="Choose the features you need and receive an estimated website budget."
          />
        )}
